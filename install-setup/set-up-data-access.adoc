...
ASA r2 存储单元默认采用精简配置。迁移虚拟机工作负载时，虚拟磁盘（VMDK）也应该进行精简配置。
...
* 详细了解link:https://kb.netapp.com/on-prem/ontap/da/SAN/SAN-KBs/How_to_setup_Broadcom_VMware_ESXi_iSCSI_MPIO_with_ONTAP_SAN_ASA["如何将 Broadcom VMware ESXi iSCSI MPIO 与ONTAP SAN ASA存储系统集成"^]。
...
从 ONTAP 9.19.1 开始，FLI 支持用于后端连接的 iSCSI 协议。

* 详细了解链接： https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html[ONTAP FLI migrations^]。
