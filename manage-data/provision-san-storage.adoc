...
在ASA r2 系统中，存储单元为 SAN 主机提供存储空间，用于数据操作。存储单元指的是 SCSI 主机的 LUN 或 NVMe 主机的 NVMe 命名空间。如果您的集群配置为支持 SCSI 主机，系统会提示您创建 LUN。如果您的集群配置为支持 NVMe 主机，系统会提示您创建 NVMe 命名空间。  ASA r2 存储单元的最大容量为 128 TB。
...
+
如果您指定的操作系统上运行的主机没有预先存在的主机组，ONTAP会自动创建一个新的主机组。
