...
searchtitle: ASA r2 和其他 ONTAP 系统之间的差异 
---
= ASA r2 和其他 ONTAP 系统之间的差异
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
当创建一个存储单元（基于 LUN 或 NVMe 命名空间）时，ONTAP 会自动在相应的存储可用性区域中创建一个卷以容纳该存储单元。新创建的卷会自动放置在存储可用性区域内，以实现最佳性能和均衡的容量利用率。容量利用率会根据您的 ONTAP 版本在存储可用性区域内实现均衡。link:../administer/space-balancing.html["了解ASA r2 集群中的容量均衡"^].
