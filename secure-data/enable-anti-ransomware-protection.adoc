...
从ONTAP 9.17.1 开始，ARP 采用机器学习模型进行反勒索软件分析，从而提高了网络弹性，该模型能够以 98% 的准确率检测 SAN 环境中不断演变的勒索软件形式。ARP的机器学习模型在模拟勒索软件攻击前后都基于大量文件数据集进行了预训练。这种资源密集型训练在ONTAP之外进行，并且由此训练生成的预训练模型包含在ONTAP中。此模型不可访问或修改。  ARP/AI 在启用后立即生效；没有link:https://docs.netapp.com/us-en/ontap/anti-ransomware/index.html#learning-and-active-modes["学习期"^]。
...
您可以在 Storage Virtual Machine (VM) 中创建的所有存储单元上默认启用 ARP/AI。这意味着在存储虚拟机中创建的任何新存储单元都将自动启用 ARP/AI。您还可以将 ARP/AI 应用于存储 VM 中的现有存储单元。
...
如果您不想在存储虚拟机中的所有存储单元上启用 ARP/AI，您可以选择要启用的特定单元。
