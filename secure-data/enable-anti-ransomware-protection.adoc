...
summary: 启用带有 AI 的 ARP，以检测潜在的勒索软件威胁，自动采取措施保护您的数据，并在 System Manager 中显示警告消息，提醒您注意可疑活动。 
---
= 在您的 ASA r2 存储系统上使用 AI 启用自主勒索软件防护
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ARP 通过采用反勒索软件分析的机器学习模型来提高网络弹性，该模型可检测 SAN 环境中不断变化的勒索软件形式。ARP 的机器学习模型在模拟勒索软件攻击之前和之后都在大型文件数据集上进行了预训练。这种资源密集型训练是在 ONTAP 外部完成的，由此训练产生的预训练模型包含在 ONTAP 的盒内。此模型不可访问或修改。ARP/AI 在启用后立即激活；没有link:https://docs.netapp.com/us-en/ontap/anti-ransomware/index.html#learning-and-active-modes["学习期"^]。
...
* 从 ONTAP 9.19.1 RC 开始，具有人工智能的自主勒索软件保护（ARP/AI）支持 ASA r2 系统上的 SnapMirror 主动同步 SAN 关系中的主读写存储单元。ARP/AI 在活动的主存储单元上运行，创建仅主 ARP 快照，并为这些存储单元提供基于机器学习的勒索软件检测。
* 从 ONTAP 9.18.1 开始，在升级到 ONTAP 9.18.1 或初始化新的 ONTAP 9.18.1 ASA r2 集群 12 小时后，默认在所有新创建的存储单元上启用 ARP/AI。
* 受 SnapLock 保护的存储单元不支持 ARP/AI。
