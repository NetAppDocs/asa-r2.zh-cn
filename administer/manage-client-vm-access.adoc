...
如果发生组件故障，LIF 可以故障转移或迁移到不同的物理端口，从而不中断网络通信。从 ONTAP 9.19.1 开始，默认情况下，IP LIF 支持自动 LIF 故障转移。启用自动 LIF 故障转移后，如果发生存储故障转移，LIF 将自动从其主节点或端口迁移到其 HA 合作伙伴节点或端口，然后在故障转移完成后返回。或者，如果端口变得不健康，LIF 将自动迁移到其当前主节点中的健康端口，然后在端口再次健康后返回其原始端口。

在 ONTAP 9.18.1 及更早版本中，对于同时具有 iSCSI 和 NVMe/TCP 流量的 IP LIF，不支持 LIF 自动故障转移。创建仅 iSCSI LIF 时，默认情况下会启用 LIF 自动故障转移，除非存储 VM 已包含未启用 LIF 自动故障转移的 iSCSI LIF。如果存储 VM 上的 iSCSI LIF 未启用 LIF 自动故障转移，则新创建的 LIF 也不会启用。在这种情况下，您必须先删除现有的 iSCSI LIF，然后重新创建它们并启用 LIF 自动故障转移。了解如何 link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["删除 SAN 环境中的 LIF"^]。
...
在 ONTAP 9.18.1 及更早版本中，如果存储虚拟机上的所有其他 iSCSI LIF 也启用了自动 LIF 故障转移，则默认情况下，当您创建仅 iSCSI LIF 时，自动 LIF 故障转移处于启用状态。如果存储虚拟机上存在未启用自动 LIF 故障转移的 iSCSI LIF，则新创建的 LIF 也不会启用自动 LIF 故障转移。

.开始之前
您必须已创建自定义的仅 iSCSI LIF 服务策略。

.关于此任务
您必须使用 ONTAP 命令行界面 (CLI) 创建仅限 iSCSI 的 LIF。
