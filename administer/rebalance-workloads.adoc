...
[role="lead"]
从ONTAP 9.17.1 开始， ONTAP会自动在ASA r2 系统 HA 对中的节点之间重新平衡工作负载，以实现最佳性能。例如，如果一个节点上的空间利用率增加到 70%，而其 HA 伙伴节点上的利用率仅为 30%，则ONTAP会自动转移工作负载，以便每个节点的空间利用率更加匹配。由于 HA 对中的节点共享相同的存储可用区，因此工作负载重新平衡是一种无副本操作，不会对性能产生任何影响。
