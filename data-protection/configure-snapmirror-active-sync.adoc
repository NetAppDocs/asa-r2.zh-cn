...
.关于此任务
如果您使用 iSCSI 或 FC 协议并使用适用于 VMware Sphere 的ONTAP工具，则可以选择link:https://docs.netapp.com/us-en/netapp-solutions/vmware/vmware-vmsc-with-smas.html["使用ONTAP Tools for VM ware 配置SnapMirror主动同步"^] 。


NOTE: 从 ONTAP 9.19.1 开始，具有人工智能的自主勒索软件保护（ARP/AI）支持 ASA r2 系统上 SnapMirror 活动同步 SAN 关系中的主读写存储单元。ARP/AI 在活动的主存储单元上运行，创建仅限主存储单元的 ARP 快照，并为这些存储单元提供基于机器学习的勒索软件检测。要启用 ARP/AI，请参见link:../secure-data/enable-anti-ransomware-protection.html["使用 AI 启用自主勒索软件防护"]。
