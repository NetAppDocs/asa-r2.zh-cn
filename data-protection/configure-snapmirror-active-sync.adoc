...
NOTE: 从 ONTAP 9.19.1 RC 开始，具有人工智能的自主勒索软件保护（ARP/AI）支持 ASA r2 系统上的 SnapMirror 主动同步 SAN 关系中的主读写存储单元。ARP/AI 在活动的主存储单元上运行，创建仅主 ARP 快照，并为这些存储单元提供基于机器学习的勒索软件检测。要启用 ARP/AI，请参阅 link:../secure-data/enable-anti-ransomware-protection.html["使用 AI 启用自主勒索软件防护"]。
