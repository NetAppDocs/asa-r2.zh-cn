...
NOTE: 采用异步策略时，数据在写入源之后才会复制到远程站点。ASA r2 系统不支持同步复制。
...
要增强防范勒索软件攻击的安全性，请创建 SnapLock for SnapVault 复制关系。
...
通过将快照复制到远程群集来保护您的数据。一致性组的复制快照可用于灾难恢复或数据迁移。
...
. （可选）要覆盖默认传输计划，请选择 *Destination settings*，然后选择 *Override transfer schedule*。
+
...
.下一步是什么？
现在已创建复制策略和关系，初始数据传输将按照复制策略中的定义开始。您可以选择测试复制故障切换，以验证 ASA r2 系统脱机时是否可以成功进行故障切换。



[[create-a-relationship-to-replicate-snapshots-with-snaplock-for-snapvault]]
=== 创建关系以使用 SnapLock for SnapVault 复制快照

为了防御勒索软件攻击，在设置复制关系时，您可以通过选择 Vault 复制策略，然后选择锁定目标 Snapshot 来为 SnapVault 启用 SnapLock。受 SnapLock for SnapVault 保护的 Snapshot 无法被意外或恶意删除。


NOTE: ASA r2 系统不支持 SnapLock Compliance 模式或 SnapLock Enterprise 模式。创建 SnapLock for SnapVault 复制关系仅允许锁定目标快照。它不启用 SnapLock Compliance 或 Enterprise 模式。

.开始之前
您必须 link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock["初始化 SnapLock 合规时钟"]。

.步骤
. 在 System Manager 中，选择 *Protection > Consistency groups*，然后选择一个一致性组。
...
.下一步是什么？
创建复制关系后，初始数据传输将按照复制策略中的定义开始。您可以选择测试复制故障切换，以验证 ASA r2 系统脱机时是否可以成功进行故障切换。
